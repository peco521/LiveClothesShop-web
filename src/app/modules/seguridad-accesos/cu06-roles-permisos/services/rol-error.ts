...
export function rolError(error: unknown): string {
  if (error instanceof HttpErrorResponse) {
    switch (error.status) {
      ...
      case 409:
        if (error.error?.error?.code === 'ultimo_usuario_cu06') return 'Debe conservar al menos un usuario activo con permiso CU06. Asigna este acceso a otro usuario elegible antes de retirarlo.';
