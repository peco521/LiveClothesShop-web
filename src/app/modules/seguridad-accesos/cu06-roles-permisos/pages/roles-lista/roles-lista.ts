...
import { rolDetalleRuta, RolDetalle, RolesListado } from '../../models/rol.models';
    ...
    @if (result(); as rows) {
      ...
      <ul>@for (rol of rows.items; track rol.nro) {
        <li><a [routerLink]="detalleRuta(rol.nro)">{{ rol.nro }} — {{ rol.descripcion }}</a>
          ...
          · {{ rol.estado === 'inactivo' ? 'Inactivo (no autoriza funciones)' : 'Activo' }}
          · <a [routerLink]="['/admin/roles', rol.nro, 'editar']">Editar descripción</a>
          @if (!rol.esRolCliente) { · <button type="button" (click)="cambiarEstado(rol)" [disabled]="busyId() === rol.nro">{{ rol.estado === 'inactivo' ? 'Reactivar' : 'Desactivar' }}</button> }</li>
...
export class RolesListaPage {
  ...
  cambiarEstado(rol: RolDetalle): void {
    if (this.loading() || rol.esRolCliente || this.busyId()) return;
    this.busyId.set(rol.nro); this.error.set('');
    this.service.estadoCuenta(rol.nro, rol.estado === 'inactivo').pipe(
      takeUntilDestroyed(this.destroy), finalize(() => this.busyId.set('')),
    ).subscribe({ next: () => this.load(this.result()?.offset ?? 0), error: (error: unknown) => this.error.set(rolError(error)) });
  }
  load(offset = 0): void {
