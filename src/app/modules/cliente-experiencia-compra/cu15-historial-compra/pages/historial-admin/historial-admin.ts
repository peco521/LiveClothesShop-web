...
@Component({ selector: 'app-historial-admin', imports: [ReactiveFormsModule, DatePipe, CurrencyPipe, CompraEstados, DetalleCompraPanel], templateUrl: './historial-admin.html',
...
export class HistorialAdminPage {
  ...
  /** Controla el panel lateral del detalle (se abre con «Ver detalle»). */
  ...
  search(offset = 0): void {
    ...
    this.selected.set(null); this.history.set(null); this.detail.set(null); this.detailOpen.set(false); this.historyError.set(''); this.detailError.set(''); this.error.set(''); this.clients.set(null); this.searching.set(true);
  ...
  loadHistory(offset = 0): void {
    ...
    this.historyRequest?.unsubscribe(); this.detailRequest?.unsubscribe(); this.history.set(null); this.detail.set(null); this.detailOpen.set(false); this.historyError.set(''); this.detailError.set(''); this.loadingHistory.set(true);
  ...
  /** Abre el panel lateral del detalle y consulta la compra solicitada. */
  ...
  /** Cierra el panel sin descartar las compras ya listadas. */
  closeDetail(): void {
    this.detailRequest?.unsubscribe();
    this.detailOpen.set(false); this.detail.set(null); this.detailError.set('');
  }
}
